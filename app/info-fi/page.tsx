    ...
    <div className="min-h-screen">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-10 animate-reveal">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold"
              style={{ background: "rgba(109,129,150,0.12)", border: "1px solid rgba(109,129,150,0.2)" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {/* Main Scanner */}
          <div className="md:col-span-2 space-y-6 animate-reveal stagger-1">
            ...
            {!hasTwitter && (
              <div className="card-static p-5 flex items-center justify-between" style={{ border: "1px solid rgba(236,72,153,0.3)" }}>
                <div>
                  ...
                  <p className="text-xs" style={{ color: "var(--color-text-secondary)" }}>Connect your Twitter to maximize scan accuracy.</p>
                </div>
            ...
            <div className="card-static p-8">
              ...
              <div className="relative mb-6">
                <input 
                  ...
                  disabled={scanning}
                />
                <IconTwitter size={20} className="absolute right-4 top-1/2 -translate-y-1/2 opacity-50" style={{ color: "var(--color-primary)" }} />
              ...
              {result && (
                <div className={`p-5 rounded-xl mb-6 ${result.success ? 'bg-primary/5 border-primary/20 border' : 'bg-red-500/10 border-red-500/20 border'}`}>
                  <div className="flex items-start gap-3">
                    {result.success ? <IconCheck size={20} style={{ color: "var(--color-success)" }} /> : <div className="text-red-400 mt-0.5">⚠️</div>}
                    <div>
                      <p className="text-sm font-semibold text-white">{result.msg}</p>
                      {result.success && result.keywords && (
                        <div className="flex flex-wrap gap-2 mt-3">
                          {result.keywords.map(kw => (
                            <span key={kw} className="bg-black/20 px-2 py-1 rounded text-xs" style={{ color: "var(--color-success)" }}>#{kw}</span>
                          ))}
                        </div>
                      )}
                    </div>
                  ...
                  {result.success && result.xp && (
                    <div className="mt-4 text-center border-t border-white/5 pt-3">
                      <span className="text-2xl font-black text-white">+{result.xp} <span style={{ color: "var(--color-primary)" }}>XP</span></span>
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="space-y-6 animate-reveal stagger-2">
            <div className="card-static p-6">
              ...
              <ul className="space-y-4 text-sm" style={{ color: "var(--color-text-secondary)" }}>
                <li className="flex gap-2"><span style={{ color: "var(--color-primary)" }}>1.</span> Paste a URL linking to a Web3-related post.</li>
                <li className="flex gap-2"><span style={{ color: "var(--color-primary)" }}>2.</span> Our AI engine fetches the content and analyzes context.</li>
                <li className="flex gap-2"><span style={{ color: "var(--color-primary)" }}>3.</span> Engine identifies keywords like POWR, DeFi, Base, AI.</li>
                <li className="flex gap-2"><span style={{ color: "var(--color-primary)" }}>4.</span> XP is awarded dynamically (+10 base, +5 per keyword).</li>
              </ul>
            </div>
          </div>
